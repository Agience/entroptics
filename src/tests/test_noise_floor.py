...
significance level ACROSS aspect ratios (the biased median-row-norm sigma^2 used
to blow up to ~40% at N>>F and ~20% at N<<F).  Deterministic seeds -> non-flaky.
"""
import math
...
from entroptics.screen import footprints
...
def _far(N, F, trials=200):
    ...
    for i in range(trials):
        ...
        hits += Screen(W).K_signal >= 1
    return hits / trials


@pytest.mark.parametrize("N,F", [(1000, 20), (500, 50), (200, 50), (128, 128), (20, 1000)])
def test_floor_far_calibrated_across_aspect_ratios(N, F):
    # Target FAR is 5%; assert well under 15% at EVERY aspect ratio.  The pre-fix
    # biased floor gave 0.40 at (1000,20) and 0.20 at (20,1000) -- this pins the fix.
...
def test_floor_detects_planted_mode():
    # A rank-1 mode a bit above the edge must be resolved at every tested shape.
...
def test_floor_holds_under_heteroscedastic_noise():
    ...
    # (the shrinkage keeps this from exploding as the below-cap whitening once did).
    hits = 0
    for i in range(150):
...
def test_tw1_survival_matches_tabulated_quantiles():
    ...
    from entroptics.screen import _tw1_sf, _TW1_UPPER_Q
...
def test_footprint_empty_when_no_signal():
    ...
    # module-level helper agrees with the Screen property
...
def test_default_null_is_mp_and_unchanged():
    # the default null provider must be mp, reproducing the historical floor and K_signal
    ...
    sc = Screen(W)
    assert Screen(W, null=nulls.mp).K_signal == sc.K_signal
...
def test_permutation_provider_is_at_least_as_conservative_on_correlated_bulk():
    ...
    for s in range(8):
        ...
        km.append(Screen(X).K_signal)                                          # default mp
        kp.append(Screen(X, null=nulls.permutation(draws=80), seed=0).K_signal)
...
def test_permutation_provider_still_resolves_a_strong_mode():
    ...
    assert Screen(X, null=nulls.permutation(draws=80), seed=0).K_signal >= 1


def test_resampling_provider_deterministic_per_seed():
    W = np.random.default_rng(13).standard_normal((100, 25))
    a = Screen(W, null=nulls.permutation(draws=50), seed=7).K_signal
    b = Screen(W, null=nulls.permutation(draws=50), seed=7).K_signal
    assert a == b


def test_non_callable_null_raises():
    from entroptics.screen import noise_floor
    sc = Screen(np.random.default_rng(0).standard_normal((40, 20)))
...
def test_custom_null_provider_callback_is_evaluated_per_screen():
    ...
    sc = Screen(W, null=lambda ctx: float(ctx.spectrum[0]) * 0.999)
    assert sc.K_signal == 1


def test_aperture_screen_threads_null_provider():
    # a caller reads K_signal via Aperture(config).screen(); the provider must reach it,
    # both as a per-call override and as the aperture's own (streaming/dynamical) provider.
    W = np.random.default_rng(1).standard_normal((80, 30))
    assert Aperture(W).screen(null=nulls.permutation(draws=40)).K_signal >= 0
    assert Aperture(W, null=nulls.permutation(draws=40)).screen().K_signal >= 0
...
def test_stateful_provider_update_runs_in_the_stream():
    ...
    assert ap.screen().K_signal >= 0                  # and the provider floors the local screen
...
def test_different_provider_per_cut_point():
    ...
    def tag(kind, base):
        ...
        return prov
    routed = by_kind(screen=tag("screen", nulls.mp),
    ...
    assert Screen(W, null=routed).K_signal == Screen(W, null={"screen": nulls.mp}).K_signal
    ...
    assert set(seen) == {"screen", "spectral", "bulk"}          # each cut point hit its own entry
...
def test_aperture_routes_screen_and_spectral_to_different_providers():
    ...
    hits = {"screen": 0, "spectral": 0}
    def count(kind):
        def prov(ctx):
            hits[kind] += 1; return nulls.mp(ctx)
        return prov
    ap = Aperture(W, null=by_kind(screen=count("screen"), spectral=count("spectral")))
    _ = ap.screen().K_signal
    _ = ap.spectral.resolved_modes
    assert hits == {"screen": 1, "spectral": 1}                 # routed apart, not shared


def test_by_kind_rejects_unknown_cut_point():
    from entroptics import by_kind
    with pytest.raises(ValueError):
        by_kind(screen=nulls.mp, bogus=nulls.mp)
...
def test_reference_null_is_deterministic_and_sharpens():
    ...
    ref = [top_spectrum_value(r.standard_normal((100, 20)), "screen") for _ in range(80)]
    prov = reference_null(ref)
    def floor(far):
        return apply_floor(prov, spectrum=None, data=None, shape=(100, 20), far=far, kind="screen")
...
def test_derived_edge_serves_arbitrary_sharp_far():
    ...
    assert Screen(W, far=1e-5).K_signal <= Screen(W, far=0.05).K_signal      # sharper -> stricter
...
def test_stateful_provider_sharpens_alpha_over_the_run():
    ...
    floors = [Screen(W, null=prov, seed=i).noise_floor for i in range(5)]
