...
from entroptics import Screen, set_precision, precision
...
def test_32_folds_in_float32():
    ...
    set_precision(32)
    s = Screen(W)
    assert s.screen.dtype == np.float32               # the fold no longer upcasts to float64
...
def test_32_preserves_k_signal_within_fp32_band():
    ...
    set_precision(64); s64 = Screen(W)
    set_precision(32); s32 = Screen(W.astype(np.float32))
    assert s32.K_signal == s64.K_signal
    assert abs(s32.sigma_top - s64.sigma_top) / s64.sigma_top < 1e-3   # fp32, not an approximation
