...
The band is closed-form, fixed by the axis lengths alone. The provenance of every constant is tabulated in §13.1.
...
Correlation against the forward model, every quantity scored on the live channels and with its per-channel baseline removed: the raw waterfall reads $0.115$–$0.373$ (mean $0.198$), the read $0.515$–$0.609$ (mean $0.559$). The read arrives smoothed along frequency and the model is smooth too, so a box-average of the raw to the same width is scored alongside it — everything the fold does and nothing else. It reads $0.215$. **The fold accounts for $5\%$ of the gain and the read for the rest.** All three columns are in the companion table, per burst.

The baseline removal is what makes the three comparable, and it is applied to all of them. `model_wfall` has a per-channel median of exactly $0$ in every channel — it is a statement about the burst and not about each channel's standing level — so a series that carried a baseline would be scored against one that does not, and the baseline would count as disagreement. Centring one series and not the others would hand that one a comparison the references never got.
...
![Figure 2. Entroptics on real CHIME/FRB Catalog-1 bursts, read untuned, four panels per event. Left to right: the raw dedispersed waterfall as measured; CHIME's own *fitburst* forward model; the entroptics reconstruction — the shrinkage against the derived floor with the persistent-structure cut, taken on the folded screen and mapped back onto the recorded frequency axis for display; and what the read did not keep, as a plain difference from the waterfall, since the read is returned on the waterfall's own amplitude scale. Per-burst $K_{\mathrm{signal}}$, contrast $\sigma_1/\Phi$, coherence $z$ and the width read at annotate each row. **Each panel is contrast-stretched independently**, between its own median and 99.7th percentile: the four hold quantities on different scales, and the model in particular is noiseless and non-negative, so the comparison to make by eye is of morphology and not of contrast. One consequence is worth naming, because it looks like an artifact and is not. The stretch of the raw panel is set by its noise, and the read removes that noise, so the reconstruction's range is several times narrower — $4.2\times$ on FRB20190106B. A channel whose standing level sits off its neighbours is then displayed against that narrower range: on FRB20190106B one channel of $10{,}972$ has a baseline $0.96$ noise sigma above the frame median, which is $0.29$ of the raw panel's range and invisible there, and $1.13$ of the reconstruction's range, where it saturates as a horizontal line. It is a real per-channel level in the record, the read reproduces it rather than inventing it (raw median $+0.134$, reconstruction $+0.126$), and it is correspondingly absent from the residual ($+0.006$, within $0.04$ standard deviations of that panel's median). Between zero and two channels per event are affected.](figures/frb_panel.png)
