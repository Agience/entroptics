...
*Decay-rate recovery (Theorem 9.2).* For a linear trajectory $x_{t+1}=Ax_t$ with known eigenvalues, `rates()` recovers $\alpha_k,\beta_k$ to machine precision at zero noise (max $|\alpha\text{ error}|=4.5\times10^{-16}$), degrading smoothly as the signal-to-noise ratio falls ($2\times10^{-2}$ at $20$ dB).
